...
class VideoEditorViewController: UIViewController {
    ...
    // These 3 view contains details will be displayed we choose effects, addText or addSticker tool
...
extension VideoEditorViewController: UINavigationControllerDelegate, UIImagePickerControllerDelegate {
    ...
    func addEffect(_ effect: Int) {
        ...
        let currentTimeInSec = CMTimeGetSeconds(videoPlayerView.currentPlayingTime)
        ...
        effectLayer.add(endVisibleFromStart, forKey: "endAnimation1")
        ...
        startVisible.beginTime = currentTimeInSec  // overlay time range start duration
        startVisible.isRemovedOnCompletion = false
        startVisible.fillMode = CAMediaTimingFillMode.forwards
        effectLayer.add(startVisible, forKey: "startAnimation")
    ...
    func addText(_ text: String, at position: Int) {
        ...
        endVisibleFromStart.fillMode = CAMediaTimingFillMode.forwards
        endVisibleFromStart.isRemovedOnCompletion = false
        textLayer.add(endVisibleFromStart, forKey: "endAnimation1")
    ...
    @objc func addAddStickerButtonHandle() {
        if let sticker = addStickerView?.sticker, let position = addStickerView?.position {
            ...
            DispatchQueue.main.async {
                let alert = UIAlertController(title: "Invalid Input", message: "Please choose a sticker and the position as well", preferredStyle: .alert)
                alert.addAction(UIAlertAction(title: "OK", style: .cancel, handler: nil))
                self.present(alert, animated: true, completion: nil)
            }
    ...
    func addSticker(_ sticker: Int, at position: Int) {
        ...
        stickerLayer.allowsGroupOpacity = true
        ...
        endVisibleFromStart.fillMode = CAMediaTimingFillMode.forwards
        endVisibleFromStart.isRemovedOnCompletion = false
        stickerLayer.add(endVisibleFromStart, forKey: "endAnimation1")
...
extension VideoEditorViewController {
    ...
    func getVideoComposition() -> AVMutableComposition {
        ...
        let mutableComposition = AVMutableComposition()
        // Get video tracks and audio tracks of our video and the AVMutableComposition
        let compositionVideoTrack = mutableComposition.addMutableTrack(withMediaType: AVMediaType.video, preferredTrackID: CMPersistentTrackID())
        let compositionAudioTrack = mutableComposition.addMutableTrack(withMediaType: AVMediaType.audio, preferredTrackID: CMPersistentTrackID())
        let videoTrack: AVAssetTrack  =  videoAsset.tracks(withMediaType: AVMediaType.video)[0]
        let audioTrack: AVAssetTrack  =  videoAsset.tracks(withMediaType: AVMediaType.audio)[0]
        
        let time = videoPlayerView.currentPlayingTime!
        print("current time of video \(time)")
        // Add our video tracks and audio tracks into the Mutable Composition normal order
        do {
