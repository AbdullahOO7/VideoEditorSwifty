    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        if keyPath == "currentItem.loadedTimeRanges" {
            ...
            if let duration = player.currentItem?.duration {
                ...
                guard !(videoDuration.isNaN || videoDuration.isInfinite) else {
                    return // or do some error handling
                }
                self.videoDurationInt = Int(self.videoDuration)
                let minutes = String(format: "%02d", self.videoDurationInt / 60)
                let seconds = String(format: "%02d", self.videoDurationInt % 60)
